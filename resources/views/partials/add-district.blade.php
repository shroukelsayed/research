...
@section('content')
                    ...
                    <tbody>
                        @foreach($districts as $district)
                            <tr>
                                ...
                                <td>{{$district->code}}</td>
                                <td>
									<button type="button" class="btn btn-info btn-sm" data-toggle="modal" data-target="#myModal" onclick="drawdev({{ $district->id}},{{$district->code }},'{{$district->name }}');"><i class="fa fa-pencil-square-o"></i> تعديل</button>
                            		<div id="addChild-{{$district->code}}"></div>
					            </td>
                            </tr>
                        @endforeach
                    </tbody>
    ...
    <script type="">
	    ...
	    function drawdev(id,code,name){
	        ...
	        html ="<div class='modal fade' id='myModal' role='dialog'><form id='form' action='{{ url("edit-district") }}' method='POST' ><div class='modal-dialog'><div class='modal-content'><div class='modal-header'><button type='button' class='close' data-dismiss='modal'>&times;</button><h4 class='modal-title'>تعديل</h4></div>";
